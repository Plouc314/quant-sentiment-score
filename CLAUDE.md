...
**Providers** — thin wrappers around external APIs (Alpaca, MediaCloud, Kaggle). Each provider fetches data and returns it as a DataFrame or TypedDict; no persistence logic.
...
**Features** — builds PyTorch-ready datasets. `TechnicalFactors` computes 16 scale-invariant OHLCV indicators. `StockDataset` aligns prices and embeddings into flat arrays with lazy windowing. `DataLoaderBuilder` applies temporal splits, fits scalers on training data, and returns DataLoaders.
...
**Top-level:**
- `models.py` — `Article`, `ArticleEncoding` TypedDicts
...
- `fetch_data.ipynb` — populate all repositories (prices, news, sentiment)
