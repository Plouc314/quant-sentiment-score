...
:class:`~sentiment.embeddings.encoder.SentimentEncoder`::
...
def fine_tune_finbert(
    ...
    *,
    ...
    in :class:`~sentiment.embeddings.encoder.SentimentEncoder` without any
    changes to downstream label logic.
...
def _load_and_prepare(fpb_config: str, tok: AutoTokenizer, seed: int) -> DatasetDict:
    ...
    # FPB ships only a "train" split; carve out a stratified validation set
