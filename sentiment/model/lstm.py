...
class SentimentLSTM(nn.Module):
    ...
    Architecture::
        ...
        classifier     : Linear(hidden_size + n_fundamentals, hidden_size)
                         → ReLU → Dropout → BatchNorm1d
                         → Linear(hidden_size, 1)

    Time-series features (technical indicators, projected sentiment embeddings,
    and FinBERT class probabilities) flow through the LSTM to capture temporal
    dynamics over the rolling window.
    ...
    repeated low-frequency values across timesteps.

    ``n_sentiment_probs=3`` enables the 3-class FinBERT probability sequence
    (``[P(pos), P(neg), P(neutral)]`` per day) in the LSTM input.  Default
    ``0`` disables it for backward compatibility when running without a
    sentiment pipeline.
    """
    ...
    def __init__(
        ...
        classifier_in = hidden_size + n_fundamentals
