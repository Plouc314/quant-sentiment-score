...
class SentimentTransformer(nn.Module):
    ...
    and FinBERT class probabilities) flow through the Transformer encoder.
    Mean pooling over the sequence dimension replaces the LSTM's final hidden
    state — the last token has no recurrent privilege in a Transformer, so mean
    pooling is more stable for short classification sequences.

    Fundamental factors (P/E, ROE, etc.) are quarterly snapshots — they are
    injected at the classifier stage alongside the pooled representation.

    ``n_sentiment_probs=3`` enables the 3-class FinBERT probability sequence
    (``[P(pos), P(neg), P(neutral)]`` per day) in the Transformer input.
    Default ``0`` disables it for backward compatibility when running without
    a sentiment pipeline.
    ...
    def __init__(
        ...
        classifier_in = d_model + n_fundamentals
    ...
    def forward(
        ...
        sentiment_probs:
            Daily FinBERT class probabilities of shape ``(batch, window, n_sentiment_probs)``,
        ...
        fused = torch.cat(parts, dim=-1)                     # (batch, window, n_factors*2 + n_sentiment_probs)
